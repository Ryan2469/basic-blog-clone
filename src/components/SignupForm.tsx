...
export default function SignupForm(){
    ...
    return(
        <form onSubmit={onSubmit} className="form form--lg">
            ...
            {error && error.length > 0 && (<div className="form__block"><div className="form__error">{error}</div></div>)}
            <div className="form__block">
                아이디가 있으신가요?
                <Link to='/login' className="form__link">로그인하기</Link> 
            </div>
            <div className="form__block">
                <input type="submit" value='제출' className="form__btn--submit" disabled={error?.length > 0}/>
            </div>
        </form>
    )
}